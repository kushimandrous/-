def f(s,m):#s куча одна  m колво ходов
    if s >= 39: return m%2==0#
    if m==0: return 0 #если ходов больше нет возвращается 0
    h = [ f(s+1,m-1), f(s+3, m-1),f(s*2,m-1)]#
    return any(h) if m%2!=0 else all(h)# если неудачный ход обязателен то any
